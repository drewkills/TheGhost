...
if(!global.pause){
	...
	for(var i = 0; i < array_length_1d(global.objective); i++){
		scr_text_small(global.objective[i],0,window_get_width()-8,8+i*41,2);
	}
	#endregion
	...
	draw_sprite(spr_paused,0,window_get_width()/2,window_get_height()/3);
	...
	for(var i = 0; i < array_length_1d(options); i++){
		...
		draw_sprite(options[i],p,window_get_width()/2+o,window_get_height()/2+i*128);
	}
}